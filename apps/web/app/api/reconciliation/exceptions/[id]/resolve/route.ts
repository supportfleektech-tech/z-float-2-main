import { resolveReconException } from "@zfloat/payments-core";
...
/** POST /api/reconciliation/exceptions/[id]/resolve — resolve with a required
 * resolution note (OPEN/INVESTIGATING/ESCALATED → RESOLVED). */
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const detail = await resolveReconException(db, { exceptionId: id, tenantId: tenantId ?? undefined, actorId, note });
